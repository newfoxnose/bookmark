...
class Teacher extends Teacher_Data
{
...
//全部书签
    public function bookmark()
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('teachers/bookmark', $data);
        $this->load->view('templates/footer');
    }

//自己的书签
    public function my_bookmark()
    {
        $data = $this->general_data;
        ...
        $data['tag'] = $this->all_model->general_select("xz_bookmark", "tag", array("is_private" => "0", "tag!=" => ""), null, null, "tag");
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('templates/header', $data);
            $this->load->view('teachers/my_bookmark', $data);
            $this->load->view('templates/footer');
        } else {
            $update_arr = array(
...
//管理目录
    public function manage_folder()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            endforeach;
            $update_arr["teacher_id"] = $_SESSION['teacher_id'];
            if ($this->input->post('submit') == "add_category") {
                $this->all_model->general_insert('xz_folder', $update_arr);
            } elseif ($this->input->post('submit') == "add_subcategory") {
                $update_arr["father_id"] = -1;
                $this->all_model->general_insert('xz_folder', $update_arr);
            } elseif ($this->input->post('submit') == "delete_subcategory") {
                $this->all_model->general_delete('xz_bookmark', array("folder_id" => $this->input->post('id')));
                $this->all_model->general_delete('xz_folder', array("id" => $this->input->post('id')));
            } elseif ($this->input->post('submit') == "delete_category") {
                //先要遍历子目录的id再删
                $this->all_model->general_delete('xz_warehouse_category', array("id" => $this->input->post('id')));
            } elseif ($this->input->post('submit') == "update_subcategory") {
                $this->all_model->general_update("xz_folder", $update_arr, array("id" => $this->input->post('id')));
            } else {
                $this->all_model->general_update("xz_folder", $update_arr, array("id" => $this->input->post('id')));
            }
